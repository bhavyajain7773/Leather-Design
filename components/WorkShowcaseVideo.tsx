      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center z-30">
            <button 
              className={`w-16 h-16 rounded-full glass border border-white/20 flex items-center justify-center transition-all duration-500 hover:scale-110 active:scale-95 ${isPlaying ? 'opacity-0 scale-90 pointer-events-none' : 'opacity-100 scale-100'}`}
              aria-label={isPlaying ? "Pause video" : "Play video"}
            >
              {isPlaying ? (
                <Pause size={20} className="text-white fill-white" />
              ) : (
                <Play size={20} className="text-white fill-white translate-x-1" />
              )}
            </button>
          </div>
